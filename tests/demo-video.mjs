...
const deliverablesVideo = path.resolve('deliverables/gecko-next-demo.webm');
...
await caption(page, '2. Импортируем JSON-предразметку: фрагменты сразу появляются прямо на waveform.');
...
await caption(page, '3. Рабочее место: waveform, цветные сегменты спикеров, список фрагментов и редактор.');
await page.waitForTimeout(1100);

await caption(page, '4. Выбираем сегмент из списка: таймлайн переносится к этому же фрагменту.');
await clickText(page, 'sample-2');
await page.waitForTimeout(700);

await caption(page, '5. Кнопка Segment воспроизводит выбранный фрагмент ровно от начала до конца.');
await clickRole(page, 'button', 'Segment');
await page.waitForTimeout(1400);

await caption(page, '6. Редактируем текст предразметки и сохраняем правку автоматически.');
const textarea = page.locator('textarea').first();
await textarea.fill('Мы проверяем JSON и CSV выгрузки для ASR и ML-пайплайна.');
await page.waitForTimeout(900);

await caption(page, '7. Уточняем границы сегмента с точностью 0.01 секунды.');
...
await caption(page, '8. AI-помощник и контроль качества показывают спорные места, но не меняют текст автоматически.');
await page.locator('.ai-panel').scrollIntoViewIfNeeded();
await page.waitForTimeout(1200);

await caption(page, '9. Термины ведутся внутри сервиса: спорные и подтверждённые значения не теряются.');
await clickRole(page, 'button', 'Термины');
await page.waitForTimeout(1200);

await caption(page, '10. Отправляем задачу на проверку после чек-листа и автоматических проверок.');
...
await caption(page, '11. Переключаемся на роль верификатора: права доступа разделены по ролям.');
...
await caption(page, '12. Верификатор выбирает фрагмент, слушает Segment и сравнивает исходный и текущий текст.');
await page.locator('.verifier-segment-panel .segment-row').nth(1).click();
await page.waitForTimeout(500);
await clickRole(page, 'button', 'Segment');
await page.waitForTimeout(1200);

await caption(page, '13. Задачу можно вернуть на доработку или принять.');
await clickRole(page, 'button', 'Принять');
await page.waitForTimeout(900);

await caption(page, '14. Экспортируем Gecko-compatible JSON для ML/ASR пайплайна.');
...
await caption(page, '15. В аналитике видны прогресс, замечания, возвраты, качество и confidence по сегментам.');
await clickRole(page, 'button', 'Аналитика');
await page.waitForTimeout(1500);

await caption(page, '16. Админ управляет ролями, правилами и блокировкой пользователей.');
await clickRole(page, 'button', 'Выйти');
await clickText(page, 'Админ Gecko');
await clickRole(page, 'button', 'Админ');
await page.waitForSelector('.admin-user-row', { timeout: 15000 });
await page.locator('.admin-user-row').first().getByRole('button').click();
await page.waitForTimeout(900);

await caption(page, 'Готово: импорт -> разметка -> QA -> верификация -> экспорт -> администрирование.');
await page.waitForTimeout(1400);
...
await mkdir(path.dirname(deliverablesVideo), { recursive: true });
await copyFile(finalVideo, deliverablesVideo);

console.log(`Demo video saved: ${finalVideo}`);
console.log(`GitHub deliverable copied: ${deliverablesVideo}`);
